...
Enhanced version (docker-v20260726):
  ...
  - Enhanced publication-quality plots with more diagnostics
...
def run_analysis(
    ...
    for i in range(Nconf):
        ...
        logger.debug(f"  Loaded 2pt[{i}]: shape={_corr[i].shape}")
        ...
        logger.debug(f"  Loaded conf {conf_id}: 2pt OK, {ope_loaded}/3 OPE components")
    ...
    # ── Step 2: Combine OPE ────────────────────────────────────────────────
    # Unpolarized gluon OPE combination:
    # O = -F_{tx} - F_{ty} + 2*F_{xy}
    _ope = -_ope_30 - _ope_31 + 2 * _ope_01
    _ope = _ope.transpose(0, 2, 1)  # (Nconf, tau, z) — tau is time, z is space
    ...
    # Save combined OPE as intermediate
    save_intermediate(
    ...
    logger.info(f"Relative-time: corr2_rel={_corr2_rel.shape}, "
                f"ope_rel={_ope_rel.shape}")
    ...
    logger.info(f"  C3 range: re=[{_corr3.real.min():.4e}, "
                f"{_corr3.real.max():.4e}]")
    logger.info(f"  C3 memory: {_corr3.nbytes / 1024**2:.1f} MB")

    # Save C3 as intermediate
    save_intermediate(
    ...
    gc.collect()
    logger.debug("Freed raw data arrays")
    ...
    logger.info(f"Resampled: corr2={corr2.shape}, ope={ope_rs.shape}, "
                f"corr3={corr3.shape}")
...
def plot_effective_mass(
    ...
    """Plot effective mass from 2pt correlator with enhanced diagnostics."""
    params = config["parameters"]
    ensemble = config["ensemble"]
    Nt = ensemble["Nt"]
    Nconf = params["Nconf"]
    ...
    # Load forward-only correlator from saved meff files
    C2pt_all = np.zeros((Nconf, Nt), dtype=float)
    meff_log_all = np.zeros((Nconf, Nt - 1), dtype=float)
    loaded_meff_confs = 0
    ...
    for i in range(Nconf):
        ...
        if meff_path.exists():
            ...
            C2pt_all[i] = data["C2pt_1d_fwd"]
            meff_log_all[i] = data["meff_log"]
            loaded_meff_confs += 1
        else:
            logger.warning(f"  2pt meff file not found for conf {conf_id}: {meff_path}")

    if loaded_meff_confs == 0:
        logger.error("No meff data loaded for effective mass plot")
        return

    # Average over configs
    C2pt_mean = C2pt_all[:loaded_meff_confs].mean(axis=0)
    C2pt_std = C2pt_all[:loaded_meff_confs].std(axis=0)
    if jack and loaded_meff_confs > 1:
        C2pt_std *= np.sqrt(loaded_meff_confs - 1)
    meff_per_conf = meff_log_all[:loaded_meff_confs]

    fig, axes = plt.subplots(1, 3, figsize=(21, 5))

    # 1. Correlator (log scale)
    ax1 = axes[0]
    ax1.errorbar(range(Nt), C2pt_mean, yerr=C2pt_std,
                fmt='o', ms=3, capsize=2, label=f'P=({Px},{Py},{Pz})')
    ax1.set_title("2pt Correlator C(t+dt, t) fwd-only")
    ax1.set_xlabel("dt")
    ax1.set_ylabel("C(dt)")
    ax1.set_yscale('log')
    ax1.legend()
    ...
    # 2. Effective mass (log, forward-only)
    ax2 = axes[1]
    meff_mean = np.nanmean(meff_per_conf, axis=0)  # (Nt-1,)
    meff_std_vals = np.nanstd(meff_per_conf, axis=0)
    if jack and loaded_meff_confs > 1:
        meff_std_vals *= np.sqrt(loaded_meff_confs - 1)
    valid = ~np.isnan(meff_mean)
    ax2.errorbar(np.arange(1, Nt)[valid], meff_mean[valid],
                yerr=meff_std_vals[valid],
                fmt='o', ms=3, capsize=2, label=f'Pz={Pz} (log)')
    ax2.set_title(f"Effective Mass (log, fwd-only, a={alttc} fm)")

    ps, pe = Nt // 6, Nt // 3
    plateau_mask = valid[ps:pe]
    if np.any(plateau_mask):
        plateau_val = np.nanmean(meff_mean[ps:pe])
        ax2.axhline(y=plateau_val, color='r', linestyle='--', alpha=0.5,
                   label=f'Plateau[{ps},{pe}]≈{plateau_val:.3f} GeV')
        ax2.legend()

    ax2.set_xlabel("dt")
    ax2.set_ylabel("m_eff [GeV]")
    ax2.grid(True, alpha=0.3)

    # 3. Effective mass per-config
    ax3 = axes[2]
    for i in range(min(loaded_meff_confs, Nconf)):
        conf_id = conf_ids[i]
        vals = meff_per_conf[i]
        mask = ~np.isnan(vals)
        ax3.plot(np.arange(1, Nt)[mask], vals[mask],
                'o-', ms=2, alpha=0.5, label=f'conf {conf_id}')
    ax3.set_title("Effective Mass per Config (log)")
    ax3.set_xlabel("dt")
    ax3.set_ylabel("m_eff [GeV]")
...
def plot_field_strength_diagnostics(
    ...
    """Plot field strength diagnostics from the saved F_{mu,nu} data.

    Shows the distribution of Tr[F*F] at each time slice to verify
    the OPE computation quality.
    """
    params = config["parameters"]
    conf_ids = params["conf_ids"]
    ensemble = config["ensemble"]
    ...
    # Load OPE data per config
    ope_data = {}
    for conf_id in conf_ids:
        # Load OPE combined data
        conf_dir = data_dir / f"conf_{conf_id}"
        ...
        if ope_01_path.exists() and ope_30_path.exists() and ope_31_path.exists():
            ope_01 = np.load(ope_01_path)["ops"]  # (Nx, Nt)
    ...
    # 1. |OPE(z,t)| for each conf
    ax1 = axes[0, 0]
    for conf_id, ope in ope_data.items():
        ax1.plot(range(Nt), np.abs(ope).sum(axis=0) / Nx,
                label=f'conf {conf_id}')
    ax1.set_title(r"Average $|O(z,t)|$ vs $t$")
    ax1.set_xlabel("t")
    ax1.set_ylabel(r"$\langle |O(z,t)| \rangle_z$")
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    # 2. Re[OPE(z,t)] vs z at fixed t=Nt/2
    ax2 = axes[0, 1]
    t_fixed = Nt // 2
    for conf_id, ope in ope_data.items():
    ...
    ax4 = axes[1, 1]
    for conf_id, ope in ope_data.items():
        ax4.plot(range(Nt), ope[0, :].real,
                label=f'conf {conf_id}')
    ax4.set_title("Re[O(z=0,t)] vs t")
    ax4.set_xlabel("t")
