...
def compute_effective_mass(
    ...
    r"""Compute effective mass from pp-projected 2pt correlator.

    Uses TWO methods:
      1. Forward-only log: m_eff(t) = -ln(C(t+1)/C(t)) * hbar*c / a
         Uses ONLY forward-propagating entries (t_snk > t_src, no wrap-around).
         More robust for baryons than cosh when signal is noisy.
      2. Cosh: a*m_eff(t) = arccosh((C(t-1)+C(t+1))/(2*C(t)))
         Includes all dt with C(t) > 0 (preserves sign, no abs!).

    Averages over source time slices EXCLUDING boundary-flipped entries
    (where t_snk < t_src after the PP sign flip).

    Args:
        corr_pp: PP-projected 2pt, shape (Nt, Nt).
        Nt: Temporal extent.
        alttc: Lattice spacing in fm.
        logger: Logger instance.

    Returns:
        dict with meff_log, meff_cosh, C2pt_1d_fwd, C2pt_1d_all, plateau estimates.
    """
    fm2GeV = 0.1973  # hbar*c in GeV*fm

    # ── Forward-only 1D correlator (no boundary contamination) ──────────────
    C_fwd = np.zeros(Nt, dtype=float)
    C_fwd_counts = np.zeros(Nt, dtype=int)
    for dt in range(2, Nt):  # dt >= 2 because deltat >= 2 in Wick
        vals = []
        for t in range(Nt - dt):  # only t_snk = t+dt > t_src
            vals.append(np.real(corr_pp[t + dt, t]))
        if vals:
            C_fwd[dt] = np.mean(vals)
            C_fwd_counts[dt] = len(vals)

    # ── All-source 1D correlator (includes boundary, for comparison) ────────
    C_all = np.zeros(Nt, dtype=float)
    for dt in range(Nt):
        vals = [np.real(corr_pp[(t + dt) % Nt, t]) for t in range(Nt)]
        C_all[dt] = np.mean(vals)

    # ── Log effective mass (forward-only, only positive values) ─────────────
    meff_log = np.full(Nt - 1, np.nan)
    meff_log_dt = np.full(Nt - 1, np.nan)
    for dt in range(2, Nt - 1):
        if C_fwd[dt] > 0 and C_fwd[dt + 1] > 0:
            ratio = C_fwd[dt + 1] / C_fwd[dt]
            if 0 < ratio < 1:
                meff_log[dt] = -np.log(ratio) * fm2GeV / alttc
                meff_log_dt[dt] = dt

    # ── Cosh effective mass (preserve sign, use forward-only) ───────────────
    meff_cosh = np.full(Nt - 2, np.nan)
    for dt in range(3, Nt - 1):
        if C_fwd[dt] > 0 and C_fwd_counts[dt-1] > 0 and C_fwd_counts[dt+1] > 0:
            arg = (C_fwd[dt - 1] + C_fwd[dt + 1]) / (2.0 * C_fwd[dt])
            if arg >= 1.0:
                meff_cosh[dt - 1] = np.arccosh(arg) * fm2GeV / alttc

    # ── Plateau estimate from log meff (most stable early plateau) ──────────
    valid_log = ~np.isnan(meff_log)
    if np.any(valid_log):
        # Find the longest stable region in [4, Nt//3]
        ps, pe = max(4, Nt // 6), Nt // 3
        plateau_vals = meff_log[ps:pe][~np.isnan(meff_log[ps:pe])]
        if len(plateau_vals) >= 2:
            meff_plateau_log = float(np.mean(plateau_vals))
            meff_plateau_std = float(np.std(plateau_vals))
        else:
            # Fall back to all valid log values
            all_valid = meff_log[valid_log]
            meff_plateau_log = float(np.mean(all_valid)) if len(all_valid) > 0 else np.nan
            meff_plateau_std = float(np.std(all_valid)) if len(all_valid) > 1 else np.nan
    else:
        meff_plateau_log = np.nan
        meff_plateau_std = np.nan

    # ── Cosh plateau ────────────────────────────────────────────────────────
    ps_cosh, pe_cosh = Nt // 4, Nt // 2
    pmask_cosh = ~np.isnan(meff_cosh[ps_cosh:pe_cosh])
    meff_plateau_cosh = float(np.mean(meff_cosh[ps_cosh:pe_cosh][pmask_cosh])) if np.any(pmask_cosh) else np.nan

    if logger:
        logger.info(f"C2pt_1d (forward-only) range: [{C_fwd[C_fwd_counts>0].min():.4e}, {C_fwd.max():.4e}]")
        logger.info(f"Effective mass (log, forward-only, a={alttc} fm):")
        for dt in range(2, min(Nt - 1, 16)):
            if not np.isnan(meff_log[dt]):
                logger.info(f"  dt={dt:3d}  C={C_fwd[dt]:.4e}  m_eff(log)={meff_log[dt]:.4f} GeV")
            else:
                logger.info(f"  dt={dt:3d}  C={C_fwd[dt]:.4e}  m_eff(log)=nan (sign/noise)")
        if not np.isnan(meff_plateau_log):
            logger.info(f"  Log plateau [{ps},{pe}]: m_eff={meff_plateau_log:.4f} ± {meff_plateau_std:.4f} GeV")
        if not np.isnan(meff_plateau_cosh):
            logger.info(f"  Cosh plateau [{ps_cosh},{pe_cosh}]: m_eff={meff_plateau_cosh:.4f} GeV")
    ...
    return {
        ...
        "C2pt_1d_fwd": C_fwd,
        "C2pt_1d_all": C_all,
        "meff_plateau_log_gev": meff_plateau_log,
        "meff_plateau_log_std": meff_plateau_std,
        "meff_plateau_cosh_gev": meff_plateau_cosh,
        "meff_plateau_range_log": [ps, pe],
        "meff_plateau_range_cosh": [ps_cosh, pe_cosh],
        "meff_log_dt": meff_log_dt,
    }
    ...
    for iconf, conf_id in enumerate(conf_ids):
        ...
        for Pz in Pz_list:
            ...
            np.savez(
                ...
                meff_log=meff_result["meff_log"],
                meff_cosh=meff_result["meff_cosh"],
                C2pt_1d_fwd=meff_result["C2pt_1d_fwd"],
                C2pt_1d_all=meff_result["C2pt_1d_all"],
                meff_plateau_log_gev=meff_result["meff_plateau_log_gev"],
                meff_plateau_log_std=meff_result["meff_plateau_log_std"],
            )
            ...
            conf_results[Pz] = {
                ...
                "meff_plateau_log_gev": meff_result["meff_plateau_log_gev"],
                "meff_plateau_log_std": meff_result["meff_plateau_log_std"],
                "meff_plateau_cosh_gev": meff_result["meff_plateau_cosh_gev"],
            }
        ...
        if result["status"] == "ok":
            for Pz, r in result["results"].items():
                logger.info(
                    ...
                    f"meff(log)≈{r['meff_plateau_log_gev']:.4f} GeV, "
                    f"meff(cosh)≈{r['meff_plateau_cosh_gev']:.4f} GeV"
                )
        else:
