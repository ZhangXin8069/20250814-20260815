#!/usr/bin/env python3
"""
Proton 2pt distillation — GPU (CuPy) accelerated, docker-v20260730.

KEY CHANGES from v20260729 (NEW DATA FORMAT):
  1. Eigenvectors: per-config per-time-slice binary files (LE f8)
     Path: {eigenvector_base}/{conf_id}/eigvecs_t{tsrc}_{conf_id}
     Format: (Nev, Nv, Nc, 2) LE f8 → (Nev, Nv, Nc) complex128 on CPU
     Loaded time-slice by time-slice to GPU.

  2. Perambulators: new path and binary format
     Path: {perambulator_base}/light/{conf_id}/
     Naming: perams.{conf_id}.{dsrc}.{tsrc}
     Format: (Nspin=4, Nev_snk, Nt, Nev_src) complex128 per file
     Each file covers ALL sink times for one (tsrc, dsrc) pair.

  3. Eigenvector smearing is OFF by default (perambulator already encodes momentum smearing).

  4. Effective mass: Multiple methods (fit_cosh, fit_exp, exp_forward, cosh).

Precision flow:
  Input (disk): binary LE f8 (float64) → CPU complex128 → GPU complex64
  GPU compute:  complex64
  Output (disk): complex64 .npy/.npz
...
# ═══════════════════════════════════════════════════════════════════════════════
# NEW: Per-config per-time-slice eigenvector reader
# ═══════════════════════════════════════════════════════════════════════════════

def _read_eigenvector_slice(filepath: str, Nev: int, Nx: int) -> np.ndarray:
    """Read one eigenvector time-slice binary file.

    Binary format: LE float64, shape (Nev, Nx*Ny*Nz, Nc, 2) → complex (Nev, Nv, Nc).
    Returns complex128 numpy array.
    """
...
def load_eigenvectors_per_config(
    ...
    Reads all Nt time slices: {eigvec_base}/{conf_id}/eigvecs_t{tsrc}_{conf_id}

    Returns: (Nt, Nev, Nx^3, 3) in compute dtype (complex64 by default).
    """
    ...
    for tsrc in range(Nt):
        ...
        if not os.path.exists(fpath):
            missing.append(tsrc)
            continue
        ev_slice = _read_eigenvector_slice(fpath, Nev, Nx)
    ...
    if missing:
        ...
        if n_missing > Nt // 2:
            raise FileNotFoundError(f"Too many missing eigenvector files ({n_missing}/{Nt})")
...
# ═══════════════════════════════════════════════════════════════════════════════
# NEW: Per-config perambulator reader (updated binary format)
# ═══════════════════════════════════════════════════════════════════════════════
...
def _read_perambulator_file(filepath: str, Nev: int, Nt: int) -> np.ndarray:
    ...
    Returns complex128 numpy array of shape (Nt, Nev, 4, Nev).
    """
    raw = np.fromfile(filepath, dtype='<f8')
    Nspin = 4
...
def read_perambulator_single_t_new(
    ...
    # Transpose to match OLD: (Nt, Nspin=4, Ndsrc=4, Nev_src, Nev_snk)
...
def compute_vvv_single_t_gpu(
    eigvecs_t_gpu: cp.ndarray,      # (Nev, Nx^3, 3)
...
def compute_vvv_all_t_gpu(
    ...
    phase_gpu: cp.ndarray,          # (Nx^3,) TOTAL phase on GPU
...
def compute_wick_and_project_gpu(
    ...
    for t_src in range(Nt):
        ...
        # NEW: use updated perambulator reader
        peram_u = read_perambulator_single_t_new(peram_dir, conf_id, t_src, Nev, Nt, logger)
    ...
    # Anti-periodic BC fix for fermions
    for ts in range(Nt):
        for tk in range(Nt):
            if tk < ts:
                corr_pp[tk, ts] *= -1.0
...
def compute_effective_mass(
    ...
    Methods: cosh, exp_forward, fit_cosh, fit_exp
    """
    fm2GeV = 0.1973
...
def run_2pt_computation_gpu(config: dict, output_dir: Path, logger) -> dict:
    """Full 2pt distillation with GPU acceleration (v20260730 — new data paths)."""
    ...
    print_banner("Step 01: Proton 2pt Distillation (GPU, v20260730)", logger)
...
def main():
    p = argparse.ArgumentParser(description="Proton 2pt distillation (GPU, v20260730)")
