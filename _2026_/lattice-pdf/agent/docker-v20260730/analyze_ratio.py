...
def plot_effective_mass(
    ...
    """Plot effective mass using saved .npz files from the 2pt computation.

    The improved version (tv20260729) supports multiple meff methods
    (fit_cosh, fit_exp, exp_forward, cosh) and also loads the forward-only
    correlator C2pt_forward for comparison.
    """
    params = config["parameters"]
    ensemble = config["ensemble"]
    Nt = ensemble["Nt"]
    ...
    C2pt_fwd_all = np.zeros((Nconf, Nt))
    ...
    for i in range(Nconf):
        ...
        if meff_path.exists():
            ...
            # Load forward-only correlator if available (tv20260729+)
            if "C2pt_forward" in data:
                C2pt_fwd_all[i] = data["C2pt_forward"]
            # Load method if available
            if "method" in data:
                loaded_methods.append(str(data["method"]))
            loaded_ok += 1
        else:
    ...
    C2pt_fwd_mean = C2pt_fwd_all[:loaded_ok].mean(axis=0) if np.any(C2pt_fwd_all) else None

    method_label = loaded_methods[0] if loaded_methods else meff_method
    ...
    if np.any(neg):
        ax1.errorbar(t_vals[neg], -C2pt_mean[neg], yerr=C2pt_std[neg],
                    ...
                    label=f'C<0 (|x|, {np.sum(neg)})')
    ax1.set_title(f"2pt Correlator (source-avg), Pz={Pz}")
    ax1.set_xlabel("Delta t")
    ax1.set_ylabel("C(Delta t) [signed]")
    ax1.axhline(y=0, color='gray', lw=0.5, ls='--')
    ax1.legend(fontsize=8)
    ax1.grid(True, alpha=0.3)

    # ── Panel 2: Effective mass per config + fit ───────────────────────────
    ax2 = axes[0, 1]
    fm2GeV = 0.1973
    ...
    # Plateau estimate from saved values
    if plateau_vals:
        pv = np.array(plateau_vals)
        plateau_avg = np.mean(pv[~np.isnan(pv)])
        ax2.axhline(y=plateau_avg, color='r', linestyle='--', alpha=0.7, lw=1.5,
                   label=f'Plateau={plateau_avg:.3f} GeV [{method_label}]')
    # Proton mass reference (boosted: E = sqrt(m² + p²))
    p_phys = abs(Pz) * 2 * np.pi * fm2GeV / (Nt/72.0 * 24 * alttc)  # approx momentum in GeV...
    # Actually simplify: E_ref ≈ 1.2 GeV for Pz=-2
    ax2.axhline(y=1.2, color='g', linestyle=':', alpha=0.5, lw=1,
               label='Expected ~1.2 GeV')

    ax2.set_title(f"Effective Mass (method={method_label}, a={alttc} fm)")
    ax2.set_xlabel("t")
    ...
    # ── Panel 3: Forward-only correlator ───────────────────────────────────
    ax3 = axes[1, 0]
    if C2pt_fwd_mean is not None and np.any(np.abs(C2pt_fwd_mean) > 1e-30):
        t_fwd = np.arange(2, min(33, Nt))
        c_fwd = np.abs(C2pt_fwd_mean[2:min(33, Nt)])
        ax3.semilogy(t_fwd, c_fwd, 'bo-', ms=5, label='|C_fwd| (source-avg)')
        # Also plot a reference exponential with E=1.2 GeV
        E_ref = 1.2 * alttc / fm2GeV  # in lattice units
        ax3.semilogy(t_fwd, c_fwd[0] * np.exp(-E_ref * (t_fwd - 2)),
                    'r--', lw=1, alpha=0.5, label=f'exp(-{1.2} GeV * t)')
        ax3.set_title("Forward-only |C(Delta t)|")
        ax3.set_xlabel("Delta t")
        ax3.set_ylabel("|C_fwd(Delta t)|")
        ax3.legend(fontsize=8)
        ax3.grid(True, alpha=0.3)
    else:
        ax3.text(0.5, 0.5, "No forward-only data", ha='center', va='center',
                transform=ax3.transAxes)
        ax3.set_title("Forward-only |C(Delta t)|")

    # ── Panel 4: |C2pt| per config (log scale, for inspection) ────────────
    ax4 = axes[1, 1]
    t_zoom = min(32, Nt)
    for i in range(loaded_ok):
        vals = np.abs(C2pt_all[i, :t_zoom])
        mask = vals > 0
        ax4.semilogy(np.arange(t_zoom)[mask], vals[mask], 'o-',
                    ms=3, alpha=0.5, color=colors[i], linewidth=0.8,
                    label=f'conf {conf_ids[i]}')
    ax4.set_title(f"|C(Delta t)| per Config (t<={t_zoom})")
    ...
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    logger.info(f"Effective mass plot saved to {path} ({loaded_ok} configs, method={method_label})")
