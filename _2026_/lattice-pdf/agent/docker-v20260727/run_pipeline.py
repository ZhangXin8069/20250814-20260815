#!/usr/bin/env python3
"""
Master pipeline orchestrator — docker-v20260727 GPU-accelerated version.
...
Runs the complete GPU-accelerated pipeline:
  ...
  2. OPE computation FROM SCRATCH (GPU: CuPy F_{μν} + Wilson line + OPE)
  3. huangcl ratio analysis (Jackknife + plotting)
  4. Final report generation

Key differences from v20260726 (CPU version):
  - All heavy computation on GPU via CuPy/CUDA
  - 4.5GB eigenvectors stay on CPU, streamed to GPU per time slice
  - Gauge configs (~72 MB) loaded entirely on GPU
  - Independent GPU plaquette_clover implementation
  - GPU memory tracking throughout
...
def step_final_report(
    ...
    lines.append("")
    lines.append(f"**Version**: docker-v20260727 (GPU 加速)")
...
示例:
  ...
  python run_pipeline.py --skip-analysis           # 仅计算
    ...
    logger.info("=" * 70)
    logger.info("  Gluon PDF Validation Pipeline — docker-v20260727 (GPU)")
