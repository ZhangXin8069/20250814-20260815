#!/usr/bin/env python3
"""
OPE computation from gauge configurations — GPU (CuPy) accelerated, DOUBLE PRECISION.

KEY CHANGES over v20260729:
  1. Double precision (complex128) by default.
  2. Updated gauge config paths (new directory structure).

Precision flow:
  Input (disk): gauge .lime [complex128 big-endian] → CPU complex128 → GPU complex128
  GPU compute:  Clover F_{μν}, Wilson line, OPE contract — all complex128
  Output (disk): F_{μν} .npz, OPE .npz — complex128
...
# ═══════════════════════════════════════════════════════════════════════════════
# GPU plaquette_clover (independent CuPy implementation, complex128)
# ═══════════════════════════════════════════════════════════════════════════════
...
def plaquette_clover_gpu(gauge_gpu: cp.ndarray, mu: int, nu: int, _einsum) -> cp.ndarray:
    ...
    gauge_gpu: (Nt,Nz,Ny,Nx,4,3,3) on GPU.
...
# ═══════════════════════════════════════════════════════════════════════════════
# Gauge reader (CPU complex128 → GPU complex128)
# ═══════════════════════════════════════════════════════════════════════════════
...
def _scan_gauge_data(filepath, file_size, approx_hdr, expected_bytes, expected_elems,
                     Nt, Nx, Nc) -> np.ndarray:
    """Scan for valid gauge data near the expected offset."""
    candidates = []
...
# ═══════════════════════════════════════════════════════════════════════════════
# OPE computation per component — GPU complex128
# ═══════════════════════════════════════════════════════════════════════════════
...
def compute_ope_all_configs_gpu(config: dict, output_dir: Path, logger) -> dict:
    """Compute OPE for all configs FROM SCRATCH on GPU (complex128)."""
    ...
    print_banner("Step 02: Compute OPE from Gauge Configs (GPU, complex128)", logger)
    ...
    for iconf, conf_id in enumerate(conf_ids):
        ...
        # ── Read gauge → GPU (keep complex128) ────────────────────────────
        try:
            with Timer(f"read_gauge_conf{conf_id}", logger, output_dir.parent,
                ...
                # Keep complex128 on GPU (no downcast)
                compute_dt = get_compute_dtype()
...
def main():
    p = argparse.ArgumentParser(description="Compute OPE FROM SCRATCH (GPU, complex128)")
