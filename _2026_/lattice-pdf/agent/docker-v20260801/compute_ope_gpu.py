...
CORRECTED: matches donghx Operator.py algorithm (dual F̃, proper Wilson line).

Key fix over previous version:
  O(z) = Σ_{x⊥} Tr[ F_{μν}(x+z) · W^dag(z→0) · F̃_{μν}(x) · W(0→z) ]
  where F̃ = 0.5 * ε_{μνρσ} * F_{ρσ} is the DUAL field strength.
...
def plaquette_clover_gpu(gauge_gpu: cp.ndarray, mu: int, nu: int) -> cp.ndarray:
    ...
    gauge_gpu: (Nt,Nz,Ny,Nx,4,3,3). Returns: (Nt,Nz,Ny,Nx,3,3).
    """
    e = cp.einsum
    g = gauge_gpu
    a_mu = 3 - mu
    ...
    # F = -i/8 * Σ(P_k - P_k†)  — matches donghx line 150-160
    tr = (0, 1, 2, 3, 5, 4)
...
def plaquette_clover_all_gpu(gauge_gpu: cp.ndarray) -> cp.ndarray:
    """Compute all 4x4 F_{μν} components on GPU (0-filled diagonal).

    Returns: (4, 4, Nt, Nz, Ny, Nx, 3, 3) — 32 GB at complex64 (too big!)
    Instead returns dict with (Nt,Nz,Ny,Nx,3,3) per key.
    """
    result = {}
    for mu in range(4):
        for nu in range(4):
            if mu == nu:
                continue
            result[(mu, nu)] = plaquette_clover_gpu(gauge_gpu, mu, nu)
    return result
...
_TENSOR4_NP = build_tensor4()  # CPU copy for index lookup

def compute_dual_field_strength_munu(F_dict: dict, mu: int, nu: int) -> cp.ndarray:
    """Compute F̃_{μν} = 0.5 * Σ_{ρσ} ε_{μνρσ} * F_{ρσ} on GPU.

    Uses precomputed F components; sums over non-zero ε contributions.
    Returns: (Nt,Nz,Ny,Nx,3,3) on GPU.
    """
...
def read_gauge_lime(filepath: str, Nt: int, Nx: int, Nc: int = 3) -> np.ndarray:
    """Read .lime gauge config — try tail offset, fall back to scan."""
...
# ═══════════════════════════════════════════════════════════════════════════════
# OPE computation — matches donghx operators_new_z0_mu2
# ═══════════════════════════════════════════════════════════════════════════════

def compute_ope_donghx_gpu(
    gauge_gpu: cp.ndarray,        # (Nt,Nz,Ny,Nx,4,3,3)
    ...
    Algorithm:
      C_fwd[k] = U(0)·...·U(k-1)  — cumulative forward transporter along z_dir
      O(dz) = sum_{x} Tr[ F(z) · C[dz]^dag · F_tilde(0) · C[dz] ]

    Returns: (delta_z, Nt) complex.
    """
    compute_dt = get_compute_dtype()
    ...
    # Free local F dict entries we don't need anymore
    del local_F

    # ── Step 2: OPE matching donghx's roll-based algorithm ─────────────────
    # U_z along Wilson line direction, for all positions
    ...
    for zi in range(delta_z):
        ...
        # --- ope = F(z), rolled to origin ---
        ope_tensor = cp.roll(F, -zi, axis=z_axis)

        # --- Backward Wilson: ope = ope · U^dag(z-1) · U^dag(z-2) · ... · U^dag(0) ---
        for step in range(zi):
            shift = zi - 1 - step
            U_conj = cp.roll(U_z, -shift, axis=z_axis).conj()
            # "...ab,...cb->...ac" = ope_tensor · U_conj^T (ope right-multiplied by U^dag)
        ...
        # --- Forward Wilson: ope = ope · U(0) · U(1) · ... · U(z-1) ---
        for step in range(zi):
...
def compute_ope_all_configs_gpu(config: dict, output_dir: Path, logger) -> dict:
    ...
    # Components matching Calc_ope_unpol.py with zdir=2:
    ...
    logger.info(f"  Algorithm: operators_new_z0_mu2 (F(z)·W^dag·F̃(0)·W)")
