...
def plot_effective_mass(
    ...
    # Also try loading from the saved meff files
    meff_per_conf = np.zeros((Nconf, Nt - 2))
    for i in range(Nconf):
        conf_id = conf_ids[i]
        meff_path = data_dir / f"conf_{conf_id}" / f"meff_Pz{Pz}_conf{conf_id}.npz"
        if meff_path.exists():
            data = np.load(meff_path)
            meff_per_conf[i] = data["meff_gev"]

    fig, axes = plt.subplots(1, 3, figsize=(21, 5))

    # 1. Correlator (log scale)
    ax1 = axes[0]
    ax1.errorbar(range(Nt), np.abs(diag_mean), yerr=diag_std,
    ...
    for i in range(Nconf):
        conf_id = conf_ids[i]
        ax3.plot(np.arange(1, Nt - 1), meff_per_conf[i],
                'o-', ms=2, alpha=0.5, label=f'conf {conf_id}')
    ax3.set_title("Effective Mass per Config")
    ax3.set_xlabel("t")
    ax3.set_ylabel("a m_eff [GeV]")
